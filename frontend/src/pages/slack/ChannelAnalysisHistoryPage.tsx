...
const ChannelAnalysisHistoryPage: React.FC = () => {
  ...
  const [selectedAnalysis, setSelectedAnalysis] =
    useState<StoredAnalysisResponse | null>(null)
  ...
  const fetchChannel = async () => {
    try {
      ...
      if (data.channels && Array.isArray(data.channels)) {
        ...
        if (matchedChannel) {
          ...
          if (allChannelsResponse.ok) {
            ...
            throw new Error(
              `Error fetching all channels: ${allChannelsResponse.status}`
            )
          }
        }
      }
      ...
      toast({
        title: 'Notice',
        description:
          'Could not load channel details, but analysis history is still available',
        status: 'info',
        duration: 5000,
        isClosable: true,
      })
  ...
  const fetchAnalysisHistory = async () => {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Error fetching analyses: ${response.status} ${response.statusText}`
        )
      }
          ...
          <Tbody>
            {analyses.map((analysis) => (
              <Tr
                key={analysis.id}
                bg={
                  selectedAnalysis?.id === analysis.id ? highlightBg : undefined
                }
                cursor="pointer"
                _hover={{ bg: 'gray.50' }}
              >
                ...
                <Td>
                  <Button
                    ...
                    colorScheme="purple"
                    variant={
                      selectedAnalysis?.id === analysis.id ? 'solid' : 'outline'
                    }
    ...
    if (!selectedAnalysis) {
      return (
        <Box textAlign="center" py={10}>
          <Text>
            No analysis selected. Please select an analysis from the table
            above.
          </Text>
        </Box>
      )
        ...
        <TabPanels>
          ...
          <TabPanel>
            <Box>
              ...
              <Box>
                {selectedAnalysis.channel_summary
                  .split('\n')
                  .map((paragraph, index) => (
                    <Box key={index} mb={2}>
                      {paragraph.trim() ? (
          ...
          <TabPanel>
            <Box>
              ...
              <Box>
                {selectedAnalysis.topic_analysis
                  .split('\n')
                  .map((paragraph, index) => (
                    <Box key={index} mb={2}>
                      {paragraph.trim() ? (
          ...
          <TabPanel>
            <Box>
              ...
              <Box>
                {selectedAnalysis.contributor_insights
                  .split('\n')
                  .map((paragraph, index) => (
                    <Box key={index} mb={2}>
                      {paragraph.trim() ? (
          ...
          <TabPanel>
            <Box>
              ...
              <Box>
                {selectedAnalysis.key_highlights
                  .split('\n')
                  .map((paragraph, index) => (
                    <Box key={index} mb={2}>
                      {paragraph.trim() ? (
      ...
      <Box p={6} width="100%" maxWidth="1200px" mx="auto">
        ...
        >
          ...
          <BreadcrumbItem isCurrentPage>
            <BreadcrumbLink>
              {channel?.name ? `#${channel.name} History` : 'Channel History'}
            </BreadcrumbLink>
          </BreadcrumbItem>
        </Breadcrumb>
        ...
        <Flex justifyContent="space-between" alignItems="center" mb={6}>
          ...
          <HStack spacing={3}>
            ...
            <Button
              ...
              onClick={() =>
                navigate(
                  `/dashboard/analytics/slack/channels/${workspaceId}/${channelId}/analyze`
                )
              }
            >
              New Analysis
            ...
            <Card variant="outline">
              <CardHeader>
                ...
                <Text color="gray.500" fontSize="sm" mt={1}>
                  Generated on{' '}
                  {selectedAnalysis
                    ? formatDate(selectedAnalysis.generated_at)
                    : ''}
                </Text>
              </CardHeader>
