...
const EditProfilePage: React.FC = () => {
  ...
  const updateTeamMemberships = async (name: string) => {
    try {
      ...
      const updatePromises = teamContext.teams.map(async (team) => {
        try {
          ...
          const memberResponse = await fetch(
            `${env.apiUrl}/teams/${team.id}/members?status=all`,
            {
              method: 'GET',
              credentials: 'include',
              headers: {
          ...
          if (!memberResponse.ok) {
            const standardResponse = await fetch(
              `${env.apiUrl}/teams/${team.id}/members`,
              {
                method: 'GET',
                credentials: 'include',
                headers: {
            ...
            if (!standardResponse.ok) {
              console.error(
                `Failed to get members for team ${team.id}: ${standardResponse.status}`
              )
              results.failed++
              return
            }
        ...
        for (const membership of userMembers) {
          ...
          try {
            const updateResponse = await fetch(
              `${env.apiUrl}/teams/${teamId}/members/${membership.id}`,
              {
                method: 'PUT',
                credentials: 'include',
                headers: {
            ...
            if (!updateResponse.ok) {
              ...
              console.error(
                `Failed to update display name for membership ${membership.id}: ${updateResponse.status}`,
                errorText
              )
              results.failed++
            } else {
              results.success++
              console.log(
                `Successfully updated display name for membership ${membership.id} in team ${teamId}`
              )
            }
          } catch (error) {
            console.error(
              `Error updating display name for membership ${membership.id}:`,
              error
            )
            results.failed++
          }
      ...
      console.log(
        `Team membership updates complete. Success: ${results.success}, Failed: ${results.failed}`
      )
      ...
      if (results && results.failed > 0) {
        if (results.success > 0) {
          ...
          toast({
            title: 'Profile updated with warnings',
            description:
              'Your profile was updated, but the changes failed to sync with your team memberships. Team administrators will still see your previous name.',
            status: 'warning',
            duration: 5000,
            isClosable: true,
          })
        ...
        toast({
          ...
          description:
            'Your profile has been updated successfully across all your teams',
          status: 'success',
          duration: 3000,
          isClosable: true,
        })
