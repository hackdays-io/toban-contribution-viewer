...
const ChannelList: React.FC = () => {
  ...
  const checkSyncStatus = async () => {
    ...
    try {
      ...
      console.log(
        'Sync status check:',
        data.is_syncing ? 'SYNCING' : 'NOT SYNCING',
        'Workspace status:',
      ...
      if (data.is_syncing) {
        ...
        const wasSyncing =
          isSyncing || (previousStatus && previousStatus.is_syncing)
  ...
  const syncChannels = async () => {
    ...
    try {
      ...
      toast({
        ...
        description:
          'Starting channel synchronization. This may take several minutes for large workspaces.',
        status: 'info',
        duration: 5000,
        isClosable: true,
      })
      ...
      if (!response.ok) {
        ...
        if (errorData.detail && errorData.detail.includes('missing_scope')) {
          throw new Error(
            'Missing permissions. Your Slack app may need additional scopes like channels:read, groups:read, im:read, mpim:read'
          )
        } else {
      ...
      toast({
        ...
        description:
          data.message ||
          'Channel synchronization is running in the background. The page will refresh periodically to show updated results.',
        status: 'info',
        duration: 7000,
        isClosable: true,
      })
      ...
      toast({
        title: 'Error',
        description:
          error instanceof Error ? error.message : 'Failed to sync channels',
        status: 'error',
        duration: 5000,
        isClosable: true,
      })
  ...
  const handleSelectAll = (select: boolean) => {
    if (select) {
      ...
      console.log(
        `Selecting ${selectableChannels.length} channels on current page`
      )
      ...
      console.log(
        `Unselecting ${currentPageIds.length} channels on current page`
      )
      setSelectedChannels((prev) =>
  ...
  const filteredChannels = allChannels.filter((channel) => {
    ...
    if (typeFilter !== 'all') {
      ...
      if (typeFilter === 'direct' && !['im', 'mpim'].includes(channel.type))
        return false
    }
  ...
  const totalFilteredPages = Math.max(
    1,
    Math.ceil(totalFilteredItems / pageSize)
  )
  ...
  const changePage = (newPage: number) => {
    console.log(
      `Changing page to ${newPage} (current: ${pagination.page}, totalPages: ${totalFilteredPages})`
    )
              ...
              <Th width="50px">
                <Checkbox
                  isChecked={
                    ...
                    paginatedChannels.every((channel) =>
                      selectedChannels.includes(channel.id)
                    )
                  }
                  isIndeterminate={
                    paginatedChannels.some((channel) =>
                      selectedChannels.includes(channel.id)
                    ) &&
                    !paginatedChannels.every((channel) =>
                      selectedChannels.includes(channel.id)
            ...
            {isLoading ? (
              ...
              <Tr>
                <Td colSpan={7} textAlign="center" py={8}>
                  <VStack spacing={4}>
                    <Text color="gray.500">
                      No channels found. You may need to sync channels from
                      Slack first.
                    </Text>
                    <Button
                      colorScheme="purple"
      ...
      {!isLoading && totalFilteredPages > 1 && (
        <Flex justify="space-between" align="center" mt={6}>
          <Text color="gray.600">
            ...
            to {Math.min(currentPage * pageSize, totalFilteredItems)} of{' '}
          ...
          <Box p={4} borderWidth="1px" borderRadius="md" bg="gray.50" flex="1">
            ...
            <VStack align="start" spacing={1}>
              ...
              {syncStatus.last_channel_sync && (
                <Text fontSize="sm" color="gray.600">
                  Last Updated:{' '}
                  {new Date(syncStatus.last_channel_sync).toLocaleString()}
                </Text>
              )}
              {syncStatus.is_syncing && (
                <Text fontSize="sm" color="purple.600" fontStyle="italic">
                  Channel sync is running in the background. The list will
      ...
      >
        <AlertDialogOverlay>
          <AlertDialogContent>
            ...
            <AlertDialogBody>
              <Text mb={4}>
                Some of your selected channels don't have the Toban bot
                installed. The bot needs to be in the channel to analyze
                messages.
              </Text>
              ...
              <Text mt={4} mb={2}>
                Would you like to automatically install the bot in these
                channels?
              </Text>
              <Checkbox
