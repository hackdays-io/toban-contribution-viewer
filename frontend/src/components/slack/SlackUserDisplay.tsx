...
export interface SlackUserDisplayProps {
  ...
  fetchFromSlack?: boolean;    // Optional: Whether to fetch user data from Slack API if not in DB (default: false)
...
interface UserCacheContextType {
  ...
  loading: Set<string>;
  errors: Set<string>;
  fetchUser: (userId: string, workspaceId: string, fetchFromSlack?: boolean) => Promise<SlackUser | undefined>;
...
export const SlackUserCacheProvider: React.FC<{ children: React.ReactNode, workspaceId: string }> = ({ children, workspaceId }) => {
  ...
  const fetchUser = async (userId: string, wsId: string, fetchFromSlack: boolean = false): Promise<SlackUser | undefined> => {
    ...
    try {
      const url = `${env.apiUrl}/slack/workspaces/${wsId}/users?user_ids=${encodeURIComponent(userId)}&fetch_from_slack=${fetchFromSlack}`;
  ...
  useEffect(() => {
    ...
    const fetchUserData = async () => {
      ...
      if (!context && workspaceId) {
        setIsLoading(true);
        try {
          const url = `${env.apiUrl}/slack/workspaces/${workspaceId}/users?user_ids=${encodeURIComponent(userId)}&fetch_from_slack=${fetchFromSlack}`;
        ...
        if (cachedUser) {
          ...
          setIsLoading(true);
          const fetchedUser = await context.fetchUser(userId, workspaceId, fetchFromSlack);
          if (fetchedUser) {
            setUser(fetchedUser);
          } else {
            setHasError(true);
    ...
    fetchUserData();
  }, [userId, workspaceId, context, fetchFromSlack, _skipLoading, _testUser]);
