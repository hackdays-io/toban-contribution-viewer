...
interface IntegrationContextType extends IntegrationState {
  // CRUD operations
  fetchIntegrations: (teamId: string, serviceType?: IntegrationType) => Promise<void>;
  fetchIntegration: (integrationId: string) => Promise<void>;
  createIntegration: (data: CreateIntegrationRequest) => Promise<Integration | null>;
  createSlackIntegration: (data: CreateSlackIntegrationRequest) => Promise<Integration | null>;
  updateIntegration: (integrationId: string, data: UpdateIntegrationRequest) => Promise<Integration | null>;
  
  // Resource operations
  fetchResources: (integrationId: string, resourceTypes?: ResourceType[]) => Promise<void>;
  syncResources: (integrationId: string, resourceTypes?: string[]) => Promise<boolean>;
  
  // Sharing operations
  shareIntegration: (integrationId: string, data: IntegrationShareRequest) => Promise<IntegrationShare | null>;
...
export const IntegrationProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
   ...
   */
  const fetchIntegrations = useCallback(async (teamId: string, serviceType?: IntegrationType) => {
    if (!session || !teamId) return;
    
    setState(prev => ({ ...prev, loading: true, error: null }));
    
    try {
      const result = await integrationService.getIntegrations(teamId, serviceType);
      ...
      setState(prev => {
        ...
        const integrations = teamId === teamContext.currentTeamId
          ? result
          : prev.integrations;
   ...
   */
  const fetchIntegration = useCallback(async (integrationId: string) => {
    ...
    try {
      ...
      setState(prev => {
        const integrationIndex = prev.integrations.findIndex(i => i.id === integrationId);
        ...
        if (updatedTeamIntegrations[teamId]) {
          const teamIntIndex = updatedTeamIntegrations[teamId].findIndex(i => i.id === integrationId);
          if (teamIntIndex !== -1) {
   ...
   */
  const createIntegration = useCallback(async (data: CreateIntegrationRequest): Promise<Integration | null> => {
    ...
    try {
      ...
      setState(prev => {
        ...
        if (teamIntegrations[data.team_id]) {
          teamIntegrations[data.team_id] = [...teamIntegrations[data.team_id], result];
        } else {
   ...
   */
  const createSlackIntegration = useCallback(async (data: CreateSlackIntegrationRequest): Promise<Integration | null> => {
    if (!session) return null;
  ...
  const updateIntegration = useCallback(async (
    ...
    try {
      const result = await integrationService.updateIntegration(integrationId, data);
      ...
      setState(prev => {
        ...
        if (updatedTeamIntegrations[teamId]) {
          updatedTeamIntegrations[teamId] = updatedTeamIntegrations[teamId].map(integration => 
            integration.id === integrationId ? result : integration
          );
        }
  ...
  const fetchResources = useCallback(async (
    ...
    setState(prev => ({ ...prev, loadingResources: true, resourceError: null }));
    
    try {
      const result = await integrationService.getResources(integrationId, resourceTypes);
      ...
      setState(prev => ({ 
        ...prev, 
        loadingResources: false,
        resourceError: error instanceof Error 
          ? error 
  ...
  const syncResources = useCallback(async (
    ...
    try {
      const result = await integrationService.syncResources(integrationId, resourceTypes);
   ...
   */
  const shareIntegration = useCallback(async (
    integrationId: string, 
    ...
    try {
      const result = await integrationService.shareIntegration(integrationId, data);
  ...
  const revokeShare = useCallback(async (
    ...
    try {
      const result = await integrationService.revokeShare(integrationId, teamId);
  ...
  const grantResourceAccess = useCallback(async (
    ...
    try {
      const result = await integrationService.grantResourceAccess(integrationId, resourceId, data);
   ...
   */
  const selectIntegration = useCallback((integrationId: string | null) => {
    if (!integrationId) {
      setState(prev => ({
        ...prev,
        currentIntegration: null,
