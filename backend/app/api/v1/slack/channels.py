...
async def list_channels(
    ...
    """
    try:
        logger.info(f"Fetching channels for workspace {workspace_id} with types={types}, include_archived={include_archived}, page={page}, page_size={page_size}")
        ...
        raise HTTPException(
            status_code=500,
            detail="An error occurred while retrieving channel list"
        )
...
async def sync_channels(
    ...
    sync_all_pages: int = Query(1, ge=0, le=1, description="Whether to sync all pages (1) or just the first page (0)"),
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        logger.info(f"Starting channel sync for workspace {workspace_id} with limit={limit}, sync_all_pages={sync_all}")
        ...
        logger.info(f"Completed channel sync for workspace {workspace_id}: created={created}, updated={updated}, total={total}")
        ...
        raise HTTPException(
            status_code=500,
            detail="An error occurred while syncing channels"
        )
