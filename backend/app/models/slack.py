...
analysis_channels = Table(
    'analysis_channels',
    Base.metadata,
    Column('analysis_id', UUID(as_uuid=True), ForeignKey('slackanalysis.id'), primary_key=True),
    Column('channel_id', UUID(as_uuid=True), ForeignKey('slackchannel.id'), primary_key=True)
)
...
class SlackWorkspace(Base, BaseModel):
    ...
    analyses: Mapped[List["SlackAnalysis"]] = relationship("SlackAnalysis", back_populates="workspace")
...
class SlackChannel(Base, BaseModel):
    ...
    analyses: Mapped[List["SlackAnalysis"]] = relationship(
        "SlackAnalysis",
        secondary=analysis_channels,
        back_populates="channels"
    )
    
    # Ensure uniqueness of channels per workspace
    __table_args__ = (
        Index('ix_slackchannel_workspace_id_slack_id', 'workspace_id', 'slack_id', unique=True),
    )
...
class SlackUser(Base, BaseModel):
    ...
    contributions: Mapped[List["SlackContribution"]] = relationship("SlackContribution", back_populates="user")
    
    # Ensure uniqueness of users per workspace
    __table_args__ = (
        Index('ix_slackuser_workspace_id_slack_id', 'workspace_id', 'slack_id', unique=True),
    )
...
class SlackMessage(Base, BaseModel):
    ...
    parent: Mapped[Optional["SlackMessage"]] = relationship("SlackMessage", remote_side=[id], backref="replies")
...
class SlackReaction(Base, BaseModel):
    ...
    __table_args__ = (
        Index('ix_slackreaction_message_id_user_id_emoji_name', 'message_id', 'user_id', 'emoji_name', unique=True),
    )
...
class SlackAnalysis(Base, BaseModel):
    ...
    # Foreign keys
    workspace_id = Column(UUID(as_uuid=True), ForeignKey('slackworkspace.id'), nullable=False)
    created_by_user_id = Column(UUID(as_uuid=True), nullable=True)  # User who created the analysis
    
    # Relationships
    workspace: Mapped["SlackWorkspace"] = relationship("SlackWorkspace", back_populates="analyses")
    channels: Mapped[List["SlackChannel"]] = relationship(
        "SlackChannel",
        secondary=analysis_channels,
        back_populates="analyses"
    )
    contributions: Mapped[List["SlackContribution"]] = relationship("SlackContribution", back_populates="analysis")
...
class SlackContribution(Base, BaseModel):
    ...
    notable_contributions = Column(JSONB, nullable=True)  # List of message IDs and why they're notable
    
    # Insights
    insights = Column(Text, nullable=True)  # Generated insights about the user's contributions
    insights_data = Column(JSONB, nullable=True)  # Structured insights data
    
    # Foreign keys
    analysis_id = Column(UUID(as_uuid=True), ForeignKey('slackanalysis.id'), nullable=False)
    user_id = Column(UUID(as_uuid=True), ForeignKey('slackuser.id'), nullable=False)
    channel_id = Column(UUID(as_uuid=True), ForeignKey('slackchannel.id'), nullable=True)  # Null for overall score
    
    # Relationships
    analysis: Mapped["SlackAnalysis"] = relationship("SlackAnalysis", back_populates="contributions")
    user: Mapped["SlackUser"] = relationship("SlackUser", back_populates="contributions")
    ...
    __table_args__ = (
        Index('ix_slackcontribution_analysis_id_user_id_channel_id', 'analysis_id', 'user_id', 'channel_id', unique=True),
    )
