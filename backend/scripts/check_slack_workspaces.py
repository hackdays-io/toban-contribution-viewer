...
async def check_workspace_counts(db: AsyncSession) -> Dict[str, int]:
    ...
    stmt = select(func.count()).select_from(SlackWorkspace).where(
        SlackWorkspace.created_at >= thirty_days_ago
    )
    result = await db.execute(stmt)
    recent_count = result.scalar_one_or_none() or 0
    
    # Count workspaces with associated integrations
    stmt = select(func.count(SlackWorkspace.id)).distinct().where(
...
async def check_workspace_usage(db: AsyncSession) -> Dict[str, Dict]:
    ...
    for workspace in workspaces:
        # Count channels
        channel_stmt = select(func.count()).select_from(SlackChannel).where(
        ...
        selected_channel_stmt = select(func.count()).select_from(SlackChannel).where(
            SlackChannel.workspace_id == workspace.id,
        ...
        # Count messages
        message_stmt = select(func.count()).select_from(SlackMessage).where(
        ...
        last_message_stmt = select(SlackMessage.message_datetime).where(
            SlackMessage.channel_id.in_(
                select(SlackChannel.id).where(
                    SlackChannel.workspace_id == workspace.id
                )
            )
        ).order_by(desc(SlackMessage.message_datetime)).limit(1)
