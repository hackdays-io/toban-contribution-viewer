...
class MockCrossResourceReport:
    def __init__(self, id: uuid.UUID, team_id: uuid.UUID, title: str, description: str, 
...
async def test_team_id_selection():
    ...
    if team_id is None:
        logger.warning(f"Workspace {workspace_without_team.id} has null team_id, using integration.owner_team_id instead")
        team_id = integration.owner_team_id
        
        if team_id is None:
            logger.error(f"Cannot create CrossResourceReport: No valid team_id found in workspace {workspace_without_team.id} or integration {integration_id}")
            raise ValueError("Could not determine team_id for CrossResourceReport. Please check workspace and integration configuration.")
    ...
    try:
        ...
        assert False, "Should have raised an error when both team_ids are None"
