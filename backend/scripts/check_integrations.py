...
from app.models.integration import (
    Integration, IntegrationCredential, IntegrationType, 
    ResourceType, ServiceResource
)
from app.models.reports import (
    CrossResourceReport, ResourceAnalysis
)
...
async def check_integration_team_ids(db: AsyncSession) -> Dict[str, int]:
    ...
    stmt = select(func.count()).select_from(Integration).where(
        Integration.owner_team_id.is_(None)
    )
    result = await db.execute(stmt)
    null_team_id_count = result.scalar_one_or_none() or 0
...
async def check_resource_integrations(db: AsyncSession) -> Dict[str, int]:
    ...
    stmt = select(func.count()).select_from(ServiceResource).where(
...
async def check_channel_resources(db: AsyncSession) -> Dict[str, int]:
    ...
    stmt = select(func.count()).select_from(SlackChannel).where(
        SlackChannel.id.in_(
    ...
    if matched_count < total_channels:
        logger.warning(f"{total_channels - matched_count} SlackChannel records don't have corresponding ServiceResource records")
...
async def main():
    ...
    try:
        ...
        if integration_team_ids["null_team_id_count"] > 0:
            issues_found += 1
            logger.warning(f"⚠️ {integration_team_ids['null_team_id_count']} integrations have missing owner_team_id values")
