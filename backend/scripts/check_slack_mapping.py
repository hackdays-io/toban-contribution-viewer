...
async def check_workspace_team_mapping(db: AsyncSession) -> Tuple[int, int]:
    ...
    stmt = select(func.count()).select_from(SlackWorkspace).where(
        SlackWorkspace.team_id.is_(None)
    )
    result = await db.execute(stmt)
...
async def find_workspace_teams(db: AsyncSession) -> List[Dict]:
    ...
    stmt = select(SlackWorkspace).where(
        SlackWorkspace.team_id.is_(None)
    )
    result = await db.execute(stmt)
...
async def fix_workspace_team_mapping(db: AsyncSession, dry_run: bool = True) -> Tuple[int, int]:
    ...
    for workspace in workspace_teams:
        if workspace["can_fix"]:
            logger.info(f"{'Would fix' if dry_run else 'Fixing'} workspace {workspace['workspace_name']} ({workspace['workspace_id']}):")
            logger.info(f"  Setting team_id to {workspace['team_id']} ({workspace['team_name']})")
            
            if not dry_run:
                try:
                    stmt = update(SlackWorkspace).where(
                        SlackWorkspace.id == workspace["workspace_id"]
                    ).values(
            ...
            logger.warning(f"Cannot fix workspace {workspace['workspace_name']} ({workspace['workspace_id']}): No team association found")
            total_failed += 1
    ...
    try:
        ...
        for workspace in workspace_teams:
            if workspace["can_fix"]:
                ...
                logger.warning(f"❌ Workspace: {workspace['workspace_name']} ({workspace['workspace_id']}) - No team association found")
                unfixable_count += 1
        
        logger.info(f"Summary: {fixable_count} workspaces can be fixed, {unfixable_count} cannot be automatically fixed")
        ...
        if fixable_count > 0:
            if should_fix:
                ...
                logger.info(f"Dry run summary: {total_fixed} workspaces would be fixed, {total_failed} workspaces could not be fixed")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--auto-fix", action="store_true", help="Automatically fix workspace team mapping without prompting")
