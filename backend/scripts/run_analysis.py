...
from app.models.integration import (
    Integration, IntegrationType, ResourceType, ServiceResource
)
...
async def get_test_integration(db: AsyncSession) -> Optional[Dict[str, Any]]:
    ...
    channel_stmt = select(SlackChannel).where(
        SlackChannel.id == channel_resource.id
    )
...
async def test_unified_single_channel_analysis(db: AsyncSession, test_data: Dict[str, Any]) -> bool:
    """
    ...
    if team_id is None:
        ...
        if team_id is None:
            ...
            logger.error(f"Cannot create CrossResourceReport: No valid team_id found in workspace {workspace.id} or integration {integration.id}")
    ...
    try:
        await db.commit()
        logger.info(f"Successfully created test single-channel analysis with report ID {report_uuid} and analysis ID {analysis_uuid}")
        ...
        logger.error(f"Error creating test single-channel analysis: {str(e)}", exc_info=True)
        await db.rollback()
        return False


async def test_multi_channel_analysis(db: AsyncSession, test_data: Dict[str, Any]) -> bool:
    """
    ...
    try:
        await db.commit()
        logger.info(f"Successfully created test multi-channel analysis with report ID {report_uuid} and {len(analysis_ids)} analysis records")
        ...
        if not saved_report:
            logger.error(f"Could not retrieve saved multi-channel report with ID {report_uuid}")
            return False
        
        logger.info(f"Successfully retrieved saved multi-channel report: {saved_report.id}")
        
        # Verify that all ResourceAnalysis records are properly associated with the report
        stmt = select(ResourceAnalysis).where(ResourceAnalysis.cross_resource_report_id == report_uuid)
        result = await db.execute(stmt)
        analyses = result.scalars().all()
        
        if len(analyses) != len(analysis_ids):
            logger.error(f"Expected {len(analysis_ids)} analyses, but found {len(analyses)}")
            return False
        
        logger.info(f"Successfully retrieved all {len(analyses)} analyses associated with report {report_uuid}")
        ...
        logger.error(f"Error creating test multi-channel analysis: {str(e)}", exc_info=True)
        await db.rollback()
        return False
...
async def cleanup_test_data(db: AsyncSession, test_data: Dict[str, Any]) -> None:
    ...
    if single_channel_analysis_id:
        try:
            stmt = select(ResourceAnalysis).where(ResourceAnalysis.id == single_channel_analysis_id)
            result = await db.execute(stmt)
            ...
            logger.error(f"Error deleting test analysis {single_channel_analysis_id}: {str(e)}")
    
    single_channel_report_id = test_data.get("single_channel_report_id")
    if single_channel_report_id:
        try:
            stmt = select(CrossResourceReport).where(CrossResourceReport.id == single_channel_report_id)
            result = await db.execute(stmt)
            ...
            logger.error(f"Error deleting test report {single_channel_report_id}: {str(e)}")
    ...
    if multi_channel_report_id:
        try:
            stmt = select(CrossResourceReport).where(CrossResourceReport.id == multi_channel_report_id)
            result = await db.execute(stmt)
            report = result.scalar_one_or_none()
            ...
            logger.error(f"Error deleting test report {multi_channel_report_id}: {str(e)}")
...
async def main():
    ...
    try:
        ...
        # Run tests
        single_channel_success = await test_unified_single_channel_analysis(db, test_data)
