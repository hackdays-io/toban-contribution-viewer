...
def upgrade():
    ...
    if "is_personal" not in existing_columns:
        ...
        op.execute(sa.text("UPDATE team SET is_personal = false WHERE is_personal IS NULL"))
        # Make the column non-nullable after setting default values
    ...
    if "team_metadata" not in existing_columns:
        op.add_column("team", sa.Column("team_metadata", postgresql.JSONB(astext_type=sa.Text()), nullable=True))
    ...
    if "created_by_email" not in existing_columns:
        op.add_column("team", sa.Column("created_by_email", sa.String(255), nullable=True))
