...
def upgrade():
    ...
    if column_type == "uuid":
        ...
        op.add_column("team", sa.Column("created_by_user_id_new", sa.String(255), nullable=True))
        
        # Copy data from old column to new column with type casting
        op.execute(sa.text("UPDATE team SET created_by_user_id_new = created_by_user_id::text"))
        
        # Drop the old column
        op.drop_column("team", "created_by_user_id")
        
        # Rename the new column to the original name
        op.alter_column("team", "created_by_user_id_new", new_column_name="created_by_user_id")
        ...
        print(f"Column created_by_user_id already has type {column_type}, no need to convert")
...
def downgrade():
    ...
    if column_type != "uuid":
        ...
        op.add_column("team", sa.Column("created_by_user_id_new", postgresql.UUID(as_uuid=True), nullable=True))
        
        # Copy data from old column to new column with type casting
        # This may fail if the string is not a valid UUID
        op.execute(sa.text("UPDATE team SET created_by_user_id_new = created_by_user_id::uuid"))
